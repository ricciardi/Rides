...
-(UILabel*) createCueLabel {
    ...
    label.font = [UIFont fontWithName:@"Avenir" size:32.0];
    return label;
}
...
-(void)handlePan:(UIPanGestureRecognizer *)recognizer {
    ...
    if (recognizer.state == UIGestureRecognizerStateEnded) {
        ...
        if (_deleteOnDragRelease) {
            ...
            [self.delegate rideDeleted:self.Ride];
        }
        if (_markCompleteOnDragRelease) {
            // mark the item as complete and update the UI state
            self.Ride.completed = YES;
...
-(void)setRide:(Ride *)ride {
    ...
    _label.text = ride.text;
    _label.strikethrough = ride.completed;
    _itemCompleteLayer.hidden = !ride.completed;
}
